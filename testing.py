...
def generate_new_features(df,column1,column2):
    ...
    df[column1] = df[column1].apply(lambda x: x if x > 1e-4 else 1e-3)  # Replace non-positive values
    df[column2] = df[column2].apply(lambda x: x if x > 1e-4 else 1e-3)  # Replace non-positive values
...
def add_anomaly(df):
    ...
    #df['trend_strength'] = df['close'].rolling(window=10).apply(lambda x: x[-1] - x[0])  # Change in price over the window
    ...
    kmeans = KMeans(n_clusters=3, random_state=0)  # 2 clusters: Trend and Range
...
def prepare(df):
    ...
    columns = ['time', 'open', 'high', 'low', 'close','stop_losses', 'trade_max','return','anomaly','anomaly_degree_normalized','moving_avg']#['high', 'low', 'close', 'open', 'tickVolume','time','stop_losses','sell_high','trade_max']
    ...
    for symbol in symbol_list:
        try:
            ...
            if not df.empty:
                ...
                #pca = PCA(n_components=int(length_of_X/2))  # Adjust the number of components as needed
