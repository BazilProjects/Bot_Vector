...
def add_anomaly(df):
    # Feature engineering: create features such as returns and moving averages
    df['return'] = df['close'].pct_change()
    df['moving_avg'] = df['close'].rolling(window=10).mean()
    df.dropna(inplace=True)

    # Select features for training
    features = df.drop(columns=['type'])#[['return', 'moving_avg', 'candle_close']]
    ...
    # Calculate anomaly degree
    df['anomaly_degree'] = model.decision_function(X)

    # Normalize the anomaly degree
    min_degree = df['anomaly_degree'].min()
    max_degree = df['anomaly_degree'].max()
    df['anomaly_degree_normalized'] = (df['anomaly_degree'] - min_degree) / (max_degree - min_degree)
    return df
def add_stop_losse(df):
    ...
    # Add column with -1 for overbought, 1 for oversold, and 0 otherwise
    df['RSI_flag'] = df['RSI'].apply(lambda x: -1 if x > overbought_threshold else 1 if x < oversold_threshold else 0)
...
def prepare(df):
    ...
    #print(df)
    # Columns to consider
    df_new=add_anomaly(df_new)
    columns = ['time', 'open', 'high', 'low', 'close', 'tickVolume','stop_losses', 'trade_max','return','anomaly','RSI','RSI_flag']#['high', 'low', 'close', 'open', 'tickVolume','time','stop_losses','sell_high','trade_max']
    ...
    for symbol in symbol_list:
        try:
            ...
            if not df.empty:
                ...
                #print(dt)
                # Print all combinations
                df_new['Candle_close'] = df_new['close'].shift(-4)
                ...
                X_train, X_test, y_train, y_test = train_test_split(X, y_close, test_size=0.2, random_state=42, shuffle=True)
                ...
                models = {
                    ...
                    #'RandomForest': RandomForestRegressor(n_estimators=1, min_samples_leaf=2, max_depth=50, random_state=42),
                    ...
                    #'ada_boost_regressor':AdaBoostRegressor(n_estimators=7),
                ...
                for name, model in models.items():
                    ...
                    for i in range(X_test.shape[0]):
                        # Get the i-th row and reshape it to a 2D array with a single sample
