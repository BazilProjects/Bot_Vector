        ...
        try:
            ...
            if not df.empty:
                ...
                for item in symbol_list_lag:
                    if symbol in item:
                        trades_to_take= item[symbol]*testing_size
                    else:
                        trades_to_take=0.0004*testing_size
                for i in range(1,len(y_pred)):
                    pred_close=round(y_pred[i],decimal_places(df.iloc[i+1]['close']))
                    current_close = y_test.iloc[i]
                    previous_close= y_test.iloc[i-1]
                    previous_open= df.iloc[i-1]['open']
                    if current_close>previous_close and (current_close-previous_open)>trades_to_take:
                    ...
                    elif current_close<previous_close and (previous_open-current_close)>trades_to_take:
                        if pred_close<=current_close :
                            total_expected_wins=total_expected_wins+1
                        else:
                            total_expected_losses=total_expected_losses+1
                        total_expected_trade=total_expected_trade+1
                print(f'total_expected_trades:       {total_expected_trade}')
                print(f'total_expected_wins :         {total_expected_wins}')
                print(f'total_expected_losses:         {total_expected_losses}')
                print('*'*20)
                print('*'*20)
                print('*'*20)
